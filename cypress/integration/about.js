...
context('About page', () => {
  ...
  describe('Headings', () => {
    it('Should have several questions', () => {
      cy.get('main').within(() => {
        ...
        cy.findByText('Did you build the API alone?').should('exist')
        cy.findByText('Why did you build this?').should('exist')
        cy.findByText('Are you a native english speaker?').should('exist')
        cy.findByText('Technical stuff?').should('exist')
        cy.findByText('Can I contribute to the project?').should('exist')
  ...
  describe('Links', () => {
    it('Should navigate', () => {
      cy.get('main').within(() => {
        ...
        cy.findByText('PR').should('have.attr', 'href', config.github.site)
        cy.findByText('GitHub').should('have.attr', 'href', config.github.api)
        cy.findByText('email').should('have.attr', 'href', 'mailto:axelfuh@gmail.com')
      })
    })
  })
  ...
  describe('Github edit page button', () => {
    it('Should be a link to edit the page', () => {
      cy.get('a.edit-page').should('have.attr', 'href', config.github.site + '/blob/develop/src/pages/about.mdx')
    })
  })
})
